...
- Dependency installation now recognises a certificate trust failure — the package registry certificate cannot be validated, typically on a network that inspects TLS traffic — and offers to retry once with strict SSL certificate verification disabled. The prompt explains the security trade-off and defaults to No; declining reports the original failure and changes nothing. Accepting disables strict SSL for npm and Yarn, re-runs the same dependency installation once, and then restores the previous setting, whether the retry succeeded or failed. An existing `strict-ssl` setting is never overwritten or re-enabled behind your back, and no other kind of failure (connection timeouts, DNS problems, ordinary package errors) triggers the offer.
