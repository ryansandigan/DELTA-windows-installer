...
**Mitigation:** `setup.ps1` treats a download failure as an expected, clearly-explained possible outcome, not a mysterious bug — the error message points the operator at the official downloads page to obtain a current link. Whoever bumps `$Script:RequiredPostgresVersion` in the future must re-verify the URL resolves before shipping, the same way a version bump would need re-verification for Node.js if `nodejs.org`'s URL scheme ever changed.
