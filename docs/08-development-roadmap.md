...
- **Bounded automatic retry (shared with Phases 2A/2B):** a transient MSI failure gets exactly one automatic retry via `Invoke-DeltaComponentInstallWithRetry` (`lib\DeltaInstaller.Common.ps1` — attempt cap, numbering, settling delay, and retry messaging live there once; install command, validation, cleanup, and log handling stay per-component). After a non-zero exit code the existing Node.js validation runs first (a usable install is never retried); cleanup before the retry only ever removes a `Program Files\nodejs` directory that provably did not exist before the setup run and contains no `node.exe` — a pre-existing Node.js installation is never touched. Attempt 2 logs to `node-install-attempt2.log` so attempt 1's `node-install.log` survives. Exercised by `tools\test-delta-install-retry.ps1`.
...
- **Bounded automatic retry:** the observed transient EDB failure class (non-zero exit during extraction, incomplete directory, no service, no initialized data directory — see [06 — EDB installer exit codes are undocumented](06-deployment-risks.md#edb-installer-exit-codes-are-undocumented)) is recovered automatically via the shared `Invoke-DeltaComponentInstallWithRetry`: validation first (a working install is never retried), then safe cleanup gated on `Test-PostgresFreshInstallCleanupSafe` — deletion is allowed only when the target prefix, data directory, and service all provably did not exist before this setup run, no service is registered now, and the data directory never reached `initdb` (`PG_VERSION` absent) — then exactly one retry with an attempt-specific `--debugtrace` log (`postgres-install-attempt2.log`). `Resolve-PostgresPort` is hoisted out of `Install-PostgresServer` into the phase so a retry can never re-run the interactive port-conflict prompt. Existing installations, services, and clusters are never removed by any retry path. Decision predicate covered by `tools\test-delta-install-retry.ps1`. PostGIS (Phase 2B) uses the same orchestration with **no** file cleanup at all — its payload lands inside the PostgreSQL tree, so a retry simply re-runs the NSIS installer, which overwrites its own files.
